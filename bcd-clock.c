...
//
// Kris Adcock, Jan/Feb 2018. See http://danceswithferrets.org/geekblog/?p=1087
//
// To compile:
//   gcc -o bcd-clock bcd-clock.c -l bcm2835
//
// To run:
//   ./bcd-clock [brightness]         [brightness] if provided, must be a number between 1 and 15.
//
...
// Connections between Pi and MAX7219:
//   pin 1 on MAX (Data In)   -> pin 11 on Ras-Pi
//   pin 12 on MAX (Load /CS) -> pin 13 on Ras-Pi
//   pin 13 on MAX (clock)    -> pin 12 on Ras-Pi

static const int kPin_Max7219_DataIn = 17; // GPIO 17 (pin 11)
static const int kPin_Max7219_Clock  = 18; // GPIO 18 (pin 12)
static const int kPin_Max7219_CS     = 27; // GPIO 27 (pin 13)
// Note that _CS should be changed to 21 for the very oldest Raspberry Pis.
// For all other models of Raspberry Pi , _CS should be 27.
...
int main(int argc, char **argv)
{
	...
	while (1)
	{
		...
		// we regularly re-setup the screen, just in case of an intermittent connection problem that resets the screen.
		// Not really critical, to be honest.
		SetupBasics(iBrightness);
